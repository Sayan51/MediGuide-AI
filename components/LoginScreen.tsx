            ...
            <div className="flex items-center">
                ...
                <div>
                    <div className={`text-xs font-bold uppercase ${hasApiKey ? 'text-emerald-700' : 'text-amber-700'}`}>
                        {hasApiKey ? 'API Key Linked' : 'API Key Required'}
                    </div>
                    <div className="text-[10px] text-gray-500">
                        {hasApiKey ? 'Using linked billing project' : 'Link a project to enable Gemini 3 Pro'}
                    </div>
                </div>
            </div>
            <button 
                onClick={handleSelectKey}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${hasApiKey ? 'bg-white text-emerald-700 hover:bg-emerald-100' : 'bg-amber-600 text-white hover:bg-amber-700'}`}
            >
                {hasApiKey ? 'Change Key' : 'Link Key'}
            </button>
        </div>
